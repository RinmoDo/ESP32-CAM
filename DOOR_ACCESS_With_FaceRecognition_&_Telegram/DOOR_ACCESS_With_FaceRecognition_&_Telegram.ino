...
#define BOTtoken "1004357690:AAHhiKFsqnjbP3Wf7tTUeOlDf05-xQqyhE4"  // your Bot Token (Get from Botfather)
...
int Flag = 0;
String IpAdd;
int k = 0;  // index of last message recived
...
long interval = 10000;           // open lock for 10 000 milliseconds
bool face_recognised = false;  //var flag
...
void open_door(WebsocketsClient &client) 
{
  if (digitalRead(relay_pin) == LOW) {
    digitalWrite(relay_pin, HIGH); //door unlocks 
    Serial.println("Door Unlocked");
    client.send("door_open");
    door_opened_millis = millis(); // time door opened 
  }
}
...
void handleNewMessages(int numNewMessages) 
{
  ...
  for (int i=0; i<numNewMessages; i++) {
    ...
    if (text == "/CLOSE" && Flag != 0) {
      relay_STATUT = 0;
      digitalWrite(relay_pin, LOW);    // turn the LED off (LOW is the voltage level)
      bot.sendMessage(chat_id, "DOOR is CLOSED", "");
    }

    if (text == "/status" && Flag != 0) {
      if(relay_STATUT){
    ...
    if (text == "/options" && Flag != 0) {
      String keyboardJson = "[[\"/Stream\", \"/ADD_USER\"],[\"/status\"]]";
      bot.sendMessageWithReplyKeyboard(chat_id, "Choose from one of the following options", "", keyboardJson, true);
    }
      
    if (text == "/manual" && Flag != 0) {
      String keyboardJson = "[[\"/OPEN\"],[\"/CLOSE\"]]";
      bot.sendMessageWithReplyKeyboard(chat_id, "Choose from one of the following options", "", keyboardJson, true);
    }

    if (text == "/start") {
      String welcome = "Welcome to SMART_LOCK Bot, " + from_name + ".\n";
      welcome += "This bot would send you notifaction when someone get on your door.\n\n";
      welcome += "/manual : to switch the door lock to manuel\n";
      welcome += "/status : Returns current status of the door\n";
      welcome += "/options : to show more options\n";
      bot.sendMessage(chat_id, welcome, "");
        Flag = 1;
    }
    if ((text == "/ADD_USER" ||  text == "/Stream" ) && Flag != 0) {
          g_state = START_ENROLL;
          bot.sendMessage(chat_id, "u can edite or watch stream :  \n https://");
          bot.sendMessage(chat_id, String(WiFi.localIP())); 
          bot.sendMessage(chat_id, " for more settings!");
    }
  }
}
...
void MatchedFace(String xx)
{
       // Serial.print(" /***********/ Matched fct inside check");
    
        String mssg = "DOOR OPEN FOR " + xx;
...
void NoMatchedFace()
{
       // Serial.print(" /***********/ NoMatched fct inside check");
    
        String mssg = "New Guest infront of the DOOR !\n";
        bot.sendMessage(chat_id,mssg);
        take_send_photo(chat_id);
    
        String keyboardJson = "[[\"/OPEN\",\"/CLOSE\"],[\"/ADD_USER\"]]";
        bot.sendMessageWithReplyKeyboard(chat_id, "Choose from one of the following options", "", keyboardJson, true);
}
  ...
  send_face_list(client);
    ...
    //client.send("RECOGNISING");
     ...
     while (client.available()) {
        ...
        if (millis() - interval > door_opened_millis) { // current time - face recognised time > 5 secs 
          digitalWrite(relay_pin, LOW); //open relay
          Serial.println(" /***********/ DOOR Closed after opening");
        }
         ...
         if (millis() > Bot_lasttime + Bot_mtbs)  
        {
            ...
            while(numNewMessages) 
            {
              ...
              Serial.println(" /***********/ NewMessages inside check");
